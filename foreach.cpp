...
int getAbsoluteFiles(string directory, queue<string> &filesAbsolutePath) // 参数1[in]要变量的目录  参数2[out]存储文件名
{
    ...
    while ((d_ent = readdir(dir)) != NULL) // 一行一行的读目录下的东西,这个东西的属性放到dirent的变量中
    {
        if ((strcmp(d_ent->d_name, dot) != 0) && (strcmp(d_ent->d_name, dotdot) != 0)) // 忽略 . 和 ..
        {
            ...
            else // 如果不是目录
            {
                ...
                {
                    // printf("%s有可写权限\n", absolutePath.c_str());
                    if (endsWithVec(absolutePath, sufVecV))
                    {
...
//     getAbsoluteFiles(filePath, fnames);
...
int getAbsoluteFilesBySuffix(string directory, queue<string> &filesAbsolutePath) // 参数1[in]要变量的目录  参数2[out]存储文件名
{
    DIR *dir = opendir(directory.c_str()); // 打开目录   DIR-->类似目录句柄的东西
    if (dir == NULL)
    {
